...
FibNode *min = NULL;
FibNode *rootHead = NULL;
FibNode *rootTail = NULL;
int node_count = 0;
...
void extractMin() {
    FibNode *z = min;
    if (!z) {
        fprintf(stderr, "extractMin: z=%p, children=%p\n", (void*)z, (void*)z->children);
        return;
    }
    if (z->children) {
...
int comp(const void *x, const void *y) {
    FibNode *node1 = *(FibNode **)x;
    FibNode *node2 = *(FibNode **)y;
